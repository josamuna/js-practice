...
const totalUniqueProject = allEmployeeProjects.reduce(
  (projectAccu, currentProject, currentIndex, allProjects) => {
...
const arr5 = [
  3, 0, 1, 0, 3, 2, 3, 3, 1, 1, 1, 0, 8, 3, 3, 7, 0, 7, 8, 8, 0, 7, 3, 0,
];
const objNum = arr5
  .map((num) => {
    return num;
  })
  .reduce((accu, curr, currentIndex, array) => {
    console.log(accu, curr, currentIndex);

    let countReapeted = 0;
    array.forEach((num) => {
      if (curr === num) {
        // New matched value found
        countReapeted++;
      }
    });
    console.log('--', countReapeted);

    return countReapeted > 1 ? accu : accu;
  }, 0);
